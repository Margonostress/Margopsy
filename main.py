...
Все, что тебе нужно для расчета, уже посчитано и передано в виде точек A, B, C, D, E, F, G, Y, K, O, U.

Важно:
- НЕ называй арканы, только "код", "энергия", "программа".
- НЕ давай общих фраз, которые подойдут всем ("каждый из нас несет", "твоя душа пришла с задачей исцеления" и т.п.).
- НЕ копируй одни и те же формулировки для разных людей.
- НЕ используй как универсальные советы связку "психотерапия, медитация, ведение дневника, саморефлексия". Эти слова можно упомянуть максимум один раз и только если это реально логично под конкретный сценарий.
- Избегай пустых слов типа "баланс", "гармония", "самоценность", если за ними нет конкретного поведения или ситуации.
...
- избегай канцелярских оборотов "в данном случае", "таким образом", "следовательно",
- текст должен быть длинным, многослойным, как разбор у опытного психолога, но без философской воды.

Структура (обязательно, но формулировки можно менять):

1. Суть родового кода.
   Объясни, что за тип задачи у человека в этом Роду именно с учетом сочетания A, C, D, O и U.
   Покажи, чем эта комбинация отличается от других (не общая теория).
   Приведи хотя бы 1-2 типичных жизненных сцены, которые описывают эту суть (семья, работа, отношения).
...
2. Родовая миссия.
   ...
   Не пиши абстрактно "исцелить Род". Опиши, как это выглядит в жизни: например, выбор другого партнера, другой модели семьи, денег, родительства.

3. Сильные стороны по Роду.
   На основе F, G, O, U и A опиши реальные ресурсы, таланты, поддерживающие программы.
   Не "у тебя большой потенциал", а конкретно: в чем сила, как это может проявляться на работе, в отношениях, в родительстве, в деньгах.
   Дай примеры поведения: что человек обычно делает чуть лучше других.
...
4. Теневые родовые программы.
   ...
   В каждом пункте опиши конкретные сценарии поведения: как человек обычно реагирует, что терпит, где сливает, что делает "по привычке", а не общие страхи.
   Избегай повторяющихся клише вроде "есть риск зависимости", "есть опасность жертвенности" без конкретики.

5. Родовые динамики.
   На основе C, D, Y, O, U опиши возможные переплетения:
   - "я вместо кого-то",
   - "я как родитель своим родителям",
   - "повторяю судьбу предка".
   Объясни, как это может проявляться в быту: в разговорах, выборе партнеров, деньгах, стиле жизни.

6. Как это обычно проявляется в жизни.
   Дай 3-5 типичных жизненных сценариев именно для такой комбинации кодов:
   - как человек обычно строит отношения,
   - как принимает решения про деньги и работу,
   - как относится к телу и здоровью,
   - как ведет себя, когда ему плохо.
   Никаких общих фраз "стремление к гармонии любой ценой" - только живые сцены и привычные паттерны.

7. Алгоритм выхода.
   6-10 шагов, жестко привязанных к описанным выше сценариям.
   Шаги должны быть конкретными действиями:
   - какие разговоры стоит провести (и с кем),
   - какие решения по деньгам изменить,
   - какие родовые роли "пересдать" (например, "вернуть маме ее ответственность"),
   - какие границы провести и в каких ситуациях.
   Не повторяй одинаковый список практик из разбора в разбор. Для разных матриц - разные акценты и действия.

8. Работа с тенями.
   Покажи 2-3 самые сильные тени, которые вытекают именно из этой матрицы.
   Назови их через поведение: например, "привычка спасать взрослых", "злость на слабость родных", "стыд за свои желания".
   Объясни, как обращаться с ними бережно и как превращать их в ресурс через конкретные шаги.

9. Вопросы для самоанализа.
   5-7 вопросов, основанных на уже описанных сценариях.
   Вопросы должны быть уникальными для этого человека: отсылайся к его паттернам, Роду, деньгам, отношениям, телу.
   Не используй универсальные конструкции типа "как я могу проявлять больше любви к себе" без привязки к истории.

В конце ответа ничего не добавляй - промо-блок придет отдельно.
"""
...
- избегай пустых фраз про "баланс", "гармонию" без примеров,
- текста много, он может занимать несколько длинных сообщений.
...
1. Главная тема 2026.
   ...
   Приведи 1-2 жизненных сюжета, как это может выглядеть на практике.
...
2. Какие родовые программы активируются.
   ...
   Без общих формулировок "будет много кармических уроков". Пиши конкретные сценарии.

3. Возможности года.
   На основе личного кода года, F, G, K:
   - где можно сделать рывок,
   - где можно выйти из повторяющихся родовых циклов,
   - как год помогает приблизиться к своей родовой миссии.
   Обязательно приведи примеры: в чем именно может проявиться рост.

4. Риски и ловушки.
   Конкретные сценарии, а не абстрактное "осторожнее со спешкой".
   Свяжи их с привычными реакциями этого человека (через A, B, D):
   - как он обычно реагирует под давлением,
   - где может сам себе подножку поставить,
   - какие повторяющиеся ошибки может усилить 2026 год.

5. Первая половина года.
   Используй код первой половины года:
   - основные задачи,
   - в чем лучше разбираться,
   - какие темы мягко завершать,
   - как это связано с Родом (по O, U, Y).

6. Вторая половина года.
   Используй код второй половины года:
   - на чем держать фокус,
   - какие решения будут особенно важными,
   - что является точкой роста по Роду,
   - какие внутренние и внешние шаги важны именно здесь.

7. Алгоритм "как прожить 2026 в плюс".
   6-10 конкретных шагов, связанных с тем, что ты описал выше.
   Не общие практики, а реальные действия:
   - какие разговоры и с кем,
   - какие решения по работе и деньгам стоит принять,
   - что поменять в отношениях,
   - как по-другому обращаться с телом и нагрузкой.
   Не повторяй один и тот же список из разбора в разбор.
...
8. Работа с тенями в 2026.
   ...
   Привяжи тени к числам матрицы и кодам года, а не к общей теории.

9. Вопросы к себе про 2026.
   5-7 вопросов, основанных именно на этом прогнозе.
   Без универсальных формулировок. Вопросы должны цеплять за конкретные сценарии:
   про Род, деньги, тело, отношения, решения, которые придется принять.
